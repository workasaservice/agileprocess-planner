-- Populate Sprint Automation Defaults for Testing
-- This script seeds capacity defaults and story templates for MotherOps-Alpha

-- First, get the project and role IDs we need
-- We'll query the existing config tables to find actual IDs

-- 1. CAPACITY DEFAULTS
-- Insert default capacity per role for MotherOps-Alpha
...
WHERE p.project_name = 'MotherOps-Alpha'
  AND p.team_name ILIKE '%Alpha%'
...
-- 2. STORY TEMPLATES
-- Insert default story templates for MotherOps-Alpha
-- Starting with a simple 2-story template (Daily Standup + Planning)

WITH project_data AS (
  SELECT p.project_id, p.team_id
  FROM config_projects p
  WHERE p.project_name = 'MotherOps-Alpha' AND p.team_name ILIKE '%Alpha%'
  LIMIT 1
)
...
WHERE project_id IN (SELECT project_id FROM config_projects WHERE project_name = 'MotherOps-Alpha')
UNION ALL
SELECT 
  'STORY_TEMPLATES',
  COUNT(*)
